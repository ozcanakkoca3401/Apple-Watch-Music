...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, handleWatchKitExtensionRequest userInfo: [AnyHashable: Any]?, reply: (([AnyHashable: Any]?) -> Void)?) {
        ...
        if let userInfo = userInfo as? [String: String] {
            let selectedMessageIndex = userInfo["audio"]
            NotificationCenter.default.post(name: Notification.Name(rawValue: "WatchWantsToCommunicateWithiPhone"), object: selectedMessageIndex)
        }
    }
